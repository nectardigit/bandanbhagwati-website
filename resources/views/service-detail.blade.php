  ...
  <div class="wrap sd-grid">
    ...
    <div class="sd-col">
      ...
      <div class="rating-card">
        <div class="score">{{ $service->rating ? number_format($service->rating, 1) : '4.8' }}</div>
        <div class="rev">{{ $service->reviews_label ?: '5k' }} + Review</div>
        <div class="clients">
          @if ($reviewers->count())
          <div class="avatars">
            @foreach ($reviewers as $rv)
              <img loading="lazy" decoding="async" src="{{ media($rv->photo) }}" alt="{{ $rv->name }}" title="{{ $rv->name }}">
            @endforeach
          </div>
          @endif
